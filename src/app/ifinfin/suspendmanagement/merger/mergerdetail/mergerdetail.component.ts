...
@Component({
  moduleId: module.id,
  selector: 'app-root',
  templateUrl: './mergerdetail.component.html'
})

export class MergerdetailComponent extends BaseComponent implements OnInit {
  // get param from url
  param = this.getRouteparam.snapshot.paramMap.get('id');

  // variable
  public NumberOnlyPattern = this._numberonlyformat;
  public mergerdetailData: any = [];
  public listmergerdetail: any = [];
  ...
  private RoleAccessCode = 'R00003340000335A';

  // API Controller
  private APIController: String = 'SuspendMerger';
  private APIControllerSuspendMergerDetail: String = 'SuspendMergerDetail';
  ...
  private APIRouteForLookupSuspendMain: String = 'GetRowsLookupForSuspendMergerDetail';
 private APIRouteForGetRowForTopRate: String = 'ExecSpTopRate';
    ...
    this.dtOptions = {
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        dtParameters.paramTamp.push({
          'p_suspend_merger_code': this.param
        });
        // end param tambahan untuk getrows dynamic
        this.dalservice.Getrows(dtParameters, this.APIControllerSuspendMergerDetail, this.APIRouteForGetRows).subscribe(resp => {
          const parse = JSON.parse(resp)
          this.listmergerdetail = parse.data;
          if (parse.data != null) {
            this.listmergerdetail.numberIndex = dtParameters.start;
          }
    ...
    this.dalservice.Getrow(this.dataTamp, this.APIController, this.APIRouteForGetRow)
      .subscribe(
        res => {
          ...
          this.valDate = parsedata.merger_date;
          const ngbGetrow = this.getrowNgb(parse.data[0]);

          if (parsedata.merger_status !== 'HOLD') {
            this.isButton = true;
          } else {
            this.isButton = false;
          }
          ...
          this.callGetrowTopRate(this.model.merger_currency_code, this.valDate)
          this.showSpinner = false;
        },
        error => {
  ...
  //#region form submit
  onFormSubmit(mergerdetailForm: NgForm, isValid: boolean) {
    ...
    this.mergerdetailData = this.JSToNumberFloats(mergerdetailForm);

    if (this.param == null) {
      this.mergerdetailData.p_merger_amount = undefined;
    }

    const usersJson: any[] = Array.of(this.mergerdetailData);
    if (this.param != null) {
      // call web service
      ...
      this.dalservice.Insert(usersJson, this.APIController, this.APIRouteForInsert)
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/suspendmanagement/submergerlist/mergerdetail', parse.code]);
            } else {
              this.swalPopUpMsg(parse.data);
            }
          },
  ...
  //#region button back
  btnBack() {
    this.route.navigate(['/suspendmanagement/submergerlist']);
    $('#datatableMergerList').DataTable().ajax.reload();
  }
  ...
  btnSelectRowSysCurrency(merger_currency_code: String) {
    this.model.merger_currency_code = merger_currency_code;
    ...
    $('#datatableLookupSuspendMain').DataTable({
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        dtParameters.paramTamp.push({
          ...
          'p_suspend_currency_code': this.model.merger_currency_code
        });
        // end param tambahan untuk getrows dynamic
  ...
  btnSelectAllLookup() {
    ...
    swal({
      ...
      if (result.value) {
        ...
        this.dalservice.Insert(this.dataTamp, this.APIControllerSuspendMergerDetail, this.APIRouteForInsert)
          .subscribe(
            res => {
              this.showSpinner = false;
              const parse = JSON.parse(res);
              if (parse.result === 1) {
                this.showNotification('bottom', 'right', 'success');
                $('#datatablesMergerDetail').DataTable().ajax.reload();
  ...
  btnDeleteAll() {
    this.checkedList = [];
    for (let i = 0; i < this.listmergerdetail.length; i++) {
      if (this.listmergerdetail[i].selectedTable) {
        this.checkedList.push(this.listmergerdetail[i].id);
      }
    }
    ...
    swal({
      ...
      if (result.value) {
        ...
        this.dalservice.Delete(this.dataTamp, this.APIControllerSuspendMergerDetail, this.APIRouteForDelete)
          .subscribe(
            res => {
              this.showSpinner = false;
              const parse = JSON.parse(res);
  ...
  selectAllTable() {
    for (let i = 0; i < this.listmergerdetail.length; i++) {
      this.listmergerdetail[i].selectedTable = this.selectedAllTable;
    }
  }

  checkIfAllTableSelected() {
    this.selectedAllTable = this.listmergerdetail.every(function (item: any) {

...
export class ReleaselistComponent extends BaseComponent implements OnInit {
  ...
  private RoleAccessCode = 'R00003350000336A';


  // API Controller
  private APIController: String = 'SuspendRelease';
    ...
    this.dtOptions = {
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        dtParameters.paramTamp.push({
          ...
          'p_release_status': this.tampStatus
        });
        // end param tambahan untuk getrows dynamic
        this.dalservice.Getrows(dtParameters, this.APIController, this.APIRouteForGetRows).subscribe(resp => {
          const parse = JSON.parse(resp)
          this.listrelease = parse.data;
          if (parse.data != null) {
            this.listrelease.numberIndex = dtParameters.start;
          }
  ...
  //#region button add
  btnAdd() {
    this.route.navigate(['/suspendmanagement/subreleaselist/releasedetail']);
  }
  //#endregion button add

  //#region button edit
  btnEdit(codeEdit: string) {
    this.route.navigate(['/suspendmanagement/subreleaselist/releasedetail', codeEdit]);
  }

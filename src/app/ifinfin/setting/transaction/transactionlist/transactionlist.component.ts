...
export class TransactionlistComponent extends BaseComponent implements OnInit {
    ...
    this.dtOptions = {
      ...
      ajax: (dtParameters: any, callback) => {
        this.dalservice.Getrows(dtParameters, this.APIController, this.APIRouteForGetRows).subscribe(resp => {
          ...
          this.listtransaction = parse.data;
          if (parse.data != null) {
            this.listtransaction.numberIndex = dtParameters.start;
          }
  ...
  //#region button add
  btnAdd() {
    this.route.navigate(['/setting/subtransactionlist/transactiondetail']);
  }
  //#endregion button add

  //#region button edit
  btnEdit(codeEdit: string) {
    this.route.navigate(['/setting/subtransactionlist/transactiondetail', codeEdit]);
  }
  ...
  btnDeleteAll() {
    ...
    for (let i = 0; i < this.listtransaction.length; i++) {
      if (this.listtransaction[i].selected) {
        this.checkedList.push(this.listtransaction[i].code);
      }
    }
  ...
  selectAllTable() {
    for (let i = 0; i < this.listtransaction.length; i++) {
      this.listtransaction[i].selected = this.selectedAllTable;
    }
  }

  checkIfAllTableSelected() {
    this.selectedAllTable = this.listtransaction.every(function (item: any) {

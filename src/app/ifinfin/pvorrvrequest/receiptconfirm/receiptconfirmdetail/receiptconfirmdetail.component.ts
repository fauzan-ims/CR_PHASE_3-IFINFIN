...
@Component({
  moduleId: module.id,
  selector: 'app-root',
  templateUrl: './receiptconfirmdetail.component.html',
})

export class ReceiptconfirmdetailComponent extends BaseComponent implements OnInit {
  // get param from url
  param = this.getRouteparam.snapshot.paramMap.get('id');

  // variable
  public NumberOnlyPattern = this._numberonlyformat;
  public listreceiptconfirmdetaildetail: any = [];
  public receiptconfirmdetailData: any = [];
  public reversalData: any = [];
  ...
  private APIControllerReceivedTransactionDetail: String = 'ReceivedTransactionDetail';
  ...
  loadData() {
    this.dtOptions = {
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        this.dalservice.Getrows(dtParameters, this.APIControllerReceivedTransactionDetail, this.APIRouteForGetRows).subscribe(resp => {
          const parse = JSON.parse(resp)
          this.listreceiptconfirmdetaildetail = parse.data;
          if (parse.data != null) {
            this.listreceiptconfirmdetaildetail.numberIndex = dtParameters.start;
          }
      ...
      .subscribe(
        res => {
          ...
          if (change) {
            ...
            this.received_exch_rate = parsedata.exch_rate
  ...
  //#region form submit
  onFormSubmit(receiptconfirmdetailForm: NgForm, isValid: boolean) {
    ...
    this.receiptconfirmdetailData = this.JSToNumberFloats(receiptconfirmdetailForm);
    const usersJson: any[] = Array.of(this.receiptconfirmdetailData);
    // call web service
  ...
  //#region button back
  btnBack() {
    this.route.navigate(['/pvorrvrequest/subreceiptconfirmlist']);
    $('#datatableReceiptConfirmList').DataTable().ajax.reload();
  }
  //#endregion  button back

  //#region button add
  btnAdd() {
    this.route.navigate(['/pvorrvrequest/subreceiptconfirmlist/receiptconfirmdetaildetail', this.param]);
  }
  //#endregion button add

  //#region button edit
  btnEdit(codeEdit: string) {
    this.route.navigate(['/pvorrvrequest/subreceiptconfirmlist/receiptconfirmdetaildetail', this.param, codeEdit]);
  }
  ...
  btnSelectRowGlLink(gl_link_code: String, gl_link_name: string, currency_code: string) {
    ...
    this.model.bank_gl_link_name = gl_link_name;
  ...
  btnDeleteAll() {
    this.checkedList = [];
    for (let i = 0; i < this.listreceiptconfirmdetaildetail.length; i++) {
      if (this.listreceiptconfirmdetaildetail[i].selectedTable) {
        this.checkedList.push(this.listreceiptconfirmdetaildetail[i].id);
      }
    }
    ...
    swal({
      ...
      if (result.value) {
        for (let J = 0; J < this.checkedList.length; J++) {
          ...
          // end param tambahan untuk getrow dynamic
          this.dalservice.Delete(this.dataTamp, this.APIControllerReceivedTransactionDetail, this.APIRouteForDelete)
            .subscribe(
              res => {
                this.showSpinner = false;
                const parse = JSON.parse(res);
  ...
  selectAllTable() {
    for (let i = 0; i < this.listreceiptconfirmdetaildetail.length; i++) {
      this.listreceiptconfirmdetaildetail[i].selectedTable = this.selectedAllTable;
    }
  }

  checkIfAllTableSelected() {
    this.selectedAllTable = this.listreceiptconfirmdetaildetail.every(function (item: any) {
  ...
  origAmount(event: any) {
    this.model.received_base_amount = (event.target.value * this.received_exch_rate);
  ...
  exchRate(event: any) {
    ...
    this.model.received_orig_amount = (this.received_base_amount / event);
    // this.model.received_exch_rate = event.target.value;
    this.received_exch_rate = event;
  }
  //#endregion exchRate

  //#region btn lookup close
  btnLookupClose() {
    this.reversalRemark = null
  }
  //#endregion btn lookup close

  //#region form submit
  onFormSubmitReversal(reversaldetailForm: NgForm, reversalIsValid: boolean) {
    // validation form submit
    if (!reversalIsValid) {
      swal({
        title: 'Warning',
    ...
    this.reversalData = this.JSToNumberFloats(reversaldetailForm);
    const usersJson: any[] = Array.of(this.reversalData);

    // call web service
    this.dalservice.ExecSp(usersJson, this.APIController, this.APIRouteForReversalRequest)
      .subscribe(
        res => {
          this.showSpinner = false;
          const parse = JSON.parse(res);
          if (parse.result === 1) {
            $('#lookupReversal').modal('hide');
            this.showNotification('bottom', 'right', 'success');
            this.callGetrow();
          } else {
            this.swalPopUpMsg(parse.data);

...
@Component({
  moduleId: module.id,
  selector: 'app-root',
  templateUrl: './rvmanualdetaildetail.component.html'
})

export class RvmanualdetaildetailComponent extends BaseComponent implements OnInit {
  // get param from url
  ...
  public rvmanualdetaildetailData: any = [];
  ...
  // API Controller
  private APIController: String = 'ReceivedVoucherDetail';
  private APIControllerReceivedVoucher: String = 'ReceivedVoucher';
      ...
      .subscribe(
        res => {
          ...
          if (change) {
            ...
            this.exch_rate = parsedata.exch_rate
            this.model.base_amount = (this.orig_amount * parsedata.exch_rate);
          }
        },
        error => {
          console.log('There was an error while Retrieving Data(API) !!!' + error);
  ...
  callGetrowHeader() {
    ...
    this.dalservice.Getrow(this.dataTamp, this.APIControllerReceivedVoucher, this.APIRouteForGetRow)
      .subscribe(
        res => {
          const parse = JSON.parse(res);
          const parsedata = parse.data[0];
          ...
          this.model.orig_currency_code = parsedata.received_orig_currency_code;
          this.model.remarks = parsedata.received_remarks;
  ...
  //#region form submit
  onFormSubmit(rvmanualdetaildetailForm: NgForm, isValid: boolean) {
    ...
    this.rvmanualdetaildetailData = this.JSToNumberFloats(rvmanualdetaildetailForm);
    const usersJson: any[] = Array.of(this.rvmanualdetaildetailData);
    if (this.params != null) {
      // call web service
      ...
      this.dalservice.Insert(usersJson, this.APIController, this.APIRouteForInsert)
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/pvorrvrequest/subrvmanuallist/rvmanualdetaildetail', this.param, parse.id]);
            } else {
              this.swalPopUpMsg(parse.data);
            }
          },
  ...
  //#region button back
  btnBack() {
    this.route.navigate(['/pvorrvrequest/subrvmanuallist/rvmanualdetail', this.param]);
